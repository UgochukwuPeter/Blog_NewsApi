...
const Featured = () => {
  ...
  return (
    <div id='games'>
      ...
      {content.map((post) => (
        <div key={post.id} data-aos="fade-right" className='featured'>
          <img src={post.urlToImage} alt="" />
          <div className='right-featured'>
            <FeaturedBtn />
            <h1 data-aos="fade-up">{post.title}</h1>
            <p data-aos="fade-up" className='text-fea-desc'>{post.description}</p>
            <SocialBtn link={post.url} postLink={post.url} postId={post.id} />
          </div>
        </div>
      ))}
    </div> 
